## This code may fail to generated a WRRE if it cannot exist with the given CAM and Roller Size
    ...
    class WaveGearCommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
        ...
        def notify(self, args):
            try:
                ...
                inputs.addValueInput('waveGeneratorDiameter', 'Wave Generator Diameter (mm)', 'mm', adsk.core.ValueInput.createByString('50'))
        ...
        def notify(self, args):
            try:
                ...
                for input in inputs:
                    ...
                    elif input.id == 'waveGeneratorDiameter':
                        wave_generator_diameter = unitsMgr.evaluateExpression(input.expression, "mm")
                ...
                wave_gen_r = wave_generator_diameter / 2
                cy_r_min = (1.1 * roller_diameter) / math.sin(math.pi / cav_num) + 2 * ecc
                cy_r = max(wave_gen_r + roller_diameter, cy_r_min)
